...
function Greeting({ name, role }: { name: string | null; role: AppRole }) {
  const labels: Record<AppRole, string> = {
    ...
    surgeon: "Surgical Workspace",
    nurse: "Nursing Station",
    receptionist: "Front Desk",
    pharmacist: "Pharmacy Console",
    lab_tech: "Laboratory Console",
    accountant: "Accounts Console",
    finance_manager: "Finance Command",
    insurance_officer: "Insurance Desk",
    ot_coordinator: "OT Coordination",
    ambulance_driver: "Ambulance Operations",
    hr_manager: "HR Console",
    dept_head: "Executive Overview",
    procurement_officer: "Procurement Console",
  ...
  return (
    <>
      ...
      <MotionGrid>
        <Kpi
          label="Pending Bills"
          value={`₹${(d.pending ?? 0).toLocaleString()}`}
          icon={Wallet}
          color="warning"
        />
        <Kpi
          label="Pharmacy Revenue"
          value={`₹${(d.pharmacyRev ?? 0).toLocaleString()}`}
          icon={Pill}
        />
...
function AdminDashboard() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const t = todayISO();
      const [opd, ipd, adm, dis, revenue, pending, bedsOcc, bedsTot, otToday, emerg] =
        await Promise.all([
          countTable("opd_visits", (q) => q.gte("created_at", t)),
          countTable("admissions", (q) => q.is("discharged_at", null)),
...
function DoctorDashboard({ senior }: { senior: boolean }) {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const apptFilter = (q: any) =>
        docId ? q.eq("doctor_id", docId).gte("scheduled_at", t) : q.gte("scheduled_at", t);
      const [appts, queue, critical] = await Promise.all([
        ...
        supabase
          ...
          .limit(8)
          .then((r) => r.data ?? []),
        countTable("clinical_alerts", (q) =>
          q.eq("severity", "critical").eq("resolved", false),
        ).catch(() => 0),
      ]);
      const waiting = queue.filter(
        (a: any) => a.status === "waiting" || a.status === "booked",
      ).length;
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <Section
          ...
          span={2}
          action={
            <Link to="/appointments" className="text-xs text-primary">
              View all
            </Link>
          }
        >
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [scripts, sales, lowStock, expiring] = await Promise.all([
        ...
        sumColumn("pharmacy_sales", "total", (q) => q.gte("created_at", t)),
        countTable("medicine_batches", (q) => q.lt("quantity", 20)),
        countTable("medicine_batches", (q) =>
          q.lt("expiry_date", new Date(Date.now() + 60 * 86400000).toISOString().slice(0, 10)),
        ).catch(() => 0),
      ]);
  ...
  return (
    <>
      <MotionGrid>
        ...
        <Kpi
          label="Sales Today"
          value={`₹${(d.sales ?? 0).toLocaleString()}`}
          icon={IndianRupee}
          color="success"
        />
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [pending, collected, reportsPending, done] = await Promise.all([
        ...
        countTable("lab_orders", (q) => q.eq("status", "completed").gte("updated_at", t)).catch(
          () => 0,
        ),
      ]);
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [revDay, revMonth, pending, expenses] = await Promise.all([
        ...
        sumColumn("bills", "pending", (q) => q.gt("pending", 0)),
        sumColumn("transactions", "amount", (q) =>
          q.eq("type", "debit").gte("created_at", m),
        ).catch(() => 0),
      ]);
  ...
  return (
    <>
      <MotionGrid>
        <Kpi
          label="Revenue Today"
          value={`₹${(d.revDay ?? 0).toLocaleString()}`}
          icon={IndianRupee}
          color="success"
        />
        <Kpi
          label="Revenue MTD"
          value={`₹${(d.revMonth ?? 0).toLocaleString()}`}
          icon={TrendingUp}
          color="accent"
        />
        <Kpi
        ...
        <Kpi
          label="Expenses MTD"
          value={`₹${(d.expenses ?? 0).toLocaleString()}`}
          icon={ArrowUpRight}
          color="danger"
        />
      </MotionGrid>
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [pending, approved, rejected, revUnder] = await Promise.all([
        ...
        sumColumn("insurance_claims", "claim_amount", (q) => q.eq("status", "pending")).catch(
          () => 0,
        ),
      ]);
  ...
  return (
    <>
      <MotionGrid>
        ...
        <Kpi
          label="Revenue Under Claims"
          value={`₹${(d.revUnder ?? 0).toLocaleString()}`}
          icon={IndianRupee}
          color="accent"
        />
      </MotionGrid>
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [emps, present, leaves, payroll] = await Promise.all([
        ...
        countTable("attendance_records", (q) =>
          q.gte("date", t.slice(0, 10)).eq("status", "present"),
        ).catch(() => 0),
