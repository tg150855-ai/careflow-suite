  ...
  async function load() {
    ...
    const [bills, adm, opd, beds, surg, emrg] = await Promise.all([
      ...
      sb
        ...
        .select("id, chief_complaint, triage")
        .in("triage", ["red", "orange"])
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold flex items-center gap-2">
          <Activity className="size-6 text-primary" /> Executive Command Center
        </h1>
        <p className="text-sm text-muted-foreground">
          Live operations across the hospital. Auto-refreshes every 15s.
        </p>
      </div>
