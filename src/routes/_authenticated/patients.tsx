    ...
    <div className="space-y-6">
      ...
      <Card className="p-2">
        ...
        <div className="divide-y">
          ...
          {patients.map((p) => (
            <div key={p.id} className="flex items-center justify-between gap-3 p-4 hover:bg-surface-muted transition-colors">
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <Link to="/patients/$id" params={{ id: p.id }} className="font-medium truncate hover:text-primary">{p.full_name}</Link>
                  <div className="text-xs text-muted-foreground flex items-center gap-3 mt-0.5">
                    ...
                    <Badge variant="outline" className="capitalize text-[10px] h-5">{p.gender}</Badge>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Button asChild variant="ghost" size="icon" title="Open patient record">
                  <Link to="/patients/$id" params={{ id: p.id }}><Eye className="size-4" /></Link>
                </Button>
                {canEdit && (
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This will permanently delete {p.full_name} and linked insurance details. Clinical records may block deletion if already linked.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => deletePatient(p)}>Delete</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                )}
        ...
        <div className="flex items-center justify-between p-3 border-t text-sm text-muted-foreground">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages}>Next<ChevronRight className="size-4" /></Button>
          </div>
        </div>
      </Card>
    </div>
