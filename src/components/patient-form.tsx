...
export const patientFormSchema = z.object({
  ...
  mobile: z.string().trim().regex(/^[0-9+\-\s()]{10,18}$/, "Enter a valid mobile number"),
  ...
  if (value.coverage_limit) {
    ...
    if (!Number.isFinite(amount) || amount < 0) {
      ctx.addIssue({ code: "custom", path: ["coverage_limit"], message: "Enter a valid coverage amount" });
    }
  }
});
...
export function buildPatientSubmission(values: PatientFormValues): PatientSubmission {
  ...
  const derivedDob = !values.dob && ageNumber !== null && Number.isFinite(ageNumber)
  ...
  const hasInsurance = !!(values.insurance_company_id || values.insurance_policy_number || values.coverage_limit || values.authorization_number);
...
export function PatientForm({
  ...
  const defaultValues = useMemo(() => patientDefaults(initialPatient, initialInsurance), [initialPatient, initialInsurance]);
  const form = useForm<PatientFormValues>({ resolver: zodResolver(patientFormSchema), defaultValues, values: defaultValues });
    ...
    <form onSubmit={form.handleSubmit(submit)} className="space-y-5">
      <Card className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Gender *">
            <Select value={form.watch("gender")} onValueChange={(v) => form.setValue("gender", v as PatientFormValues["gender"], { shouldValidate: true })}>
          ...
          <Field label="Age" error={form.formState.errors.age?.message}>
            <Input type="number" min={0} max={125} {...form.register("age")} placeholder="Auto-derived if DOB is set" />
          </Field>
          <Field label="Blood group">
            <Select value={form.watch("blood_group") || "not-recorded"} onValueChange={(v) => form.setValue("blood_group", v === "not-recorded" ? "" : v)}>
      ...
      <Card className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Aadhar / ID number"><Input {...form.register("aadhaar")} /></Field>
        </div>
      </Card>
      ...
      <Card className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Insurance company">
            <Select value={form.watch("insurance_company_id") || "none"} onValueChange={(v) => form.setValue("insurance_company_id", v === "none" ? "" : v)}>
              <SelectTrigger><SelectValue placeholder="Select company" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">No insurance company</SelectItem>
                {(insuranceCompanies ?? []).map((c) => <SelectItem key={c.id} value={c.id}>{c.name}{c.policy_type ? ` · ${c.policy_type}` : ""}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field label="Policy / member number"><Input {...form.register("insurance_policy_number")} /></Field>
          ...
          <Field label="Authorization number"><Input {...form.register("authorization_number")} /></Field>
          <Field label="Valid from"><Input type="date" {...form.register("insurance_valid_from")} /></Field>
          <Field label="Valid to"><Input type="date" {...form.register("insurance_valid_to")} /></Field>
        </div>
      </Card>
